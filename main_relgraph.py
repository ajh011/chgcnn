...
from model import MyModel, ProjectionLayer
...
def main():
    ...
    feat0 = feats_list[0]
    proj = ProjectionLayer(feat0, 64)

    for graph, feats in zip(relgraphs, feats_list):
        feats_tens = [torch.tensor(np.nan_to_num(np.array(feat[1], dtype=float))) for feat in feats]
        graph.x = proj.forward(feats_tens)
    ...
    data0 = dataset[0]
    model = MyModel(data0.x.size(-1), 0, task = args.task,
