...
def hetero_relgraph_list_from_dir(directory='cif3', root='', radius:float=8.0):
    if root == '':
        root = os. getcwd()
    ...
    for filename, fileprop in id_prop_data:
        try:
            ...
            graph['atom', 'in', 'bond'].edge_index = rel_edges['atom', 'in', 'bond']
            graph['atom', 'in', 'motif'].edge_index = rel_edges['atom', 'in', 'motif']
            graph['bond', 'touches', 'bond'].edge_index = rel_edges['bond', 'touches', 'bond']
            graph['bond', 'in', 'motif'].edge_index = rel_edges['bond', 'in', 'motif']
            graph['motif', 'touches', 'motif'].edge_index = rel_edges['motif', 'touches', 'motif']
