...
def decompose(hgraph, order_types=['atom','bond']):
    sep = []
...
def hetero_relgraph_list_from_dir(directory='cif', root='', radius:float=8.0, undirected = False):
    if root == '':
        root = os. getcwd()
    ...
    for filename, fileprop in id_prop_data:
        try:
            ...
            graph['bond'].x = torch.tensor([bond[3] for bond in bonds],dtype = torch.float)
