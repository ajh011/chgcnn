...
class CHGConv(MessagePassing):
    ...
    def forward(self, x, hedge_index, hedge_attr):
        ...
        '''
        To finish forming the message
        '''

        time3 = time.perf_counter()
        print(f'hedge_x agg time: {time3-time2}')
        message_holder = []
        #print(f'hedge_attr size: {hedge_attr.shape}')
        #print(f'hedge_xs size: {hedge_index_xs.shape}')
        hedge_attr = torch.cat([hedge_attr, hedge_index_xs], dim = 1)
        for i in hedge_index[1]:
            message_holder.append(hedge_attr[i])
        z = torch.stack(message_holder, dim=0)
